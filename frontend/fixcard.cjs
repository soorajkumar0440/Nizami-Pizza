...
// The replacement logic: we want to find <div className="pro-img-box" ...> up to its closing </div>
let startStr = '<div className="pro-img-box" onClick={() => handleViewDetails(item)} style={{ cursor: \'pointer\' }}>';
let startIdx = code.indexOf(startStr);

if (startIdx !== -1) {
    // Find the next </div> after the start string
    
    // Actually the block is:
    // <div className="pro-img-box" onClick={() => handleViewDetails(item)} style={{ cursor: 'pointer' }}>
    ...
    let regex = /<div className="pro-img-box" onClick=\{\(\) => handleViewDetails\(item\)\} style=\{\{ cursor: 'pointer' \}\}>[\s\S]*?<\/div>\s*<\/div>\s*<span className="pro-price-badge">\{item\.price\}<\/span>\s*<\/div>/g;
    
    // Instead of complex regex, let's just do a string replacement since we know the exact inner structure from view_file
    let exactOld = `<div className="pro-img-box" onClick={() => handleViewDetails(item)} style={{ cursor: 'pointer' }}>
                                    <img src={item.img} alt={item.name} className="img-fill" />
                                    <div className="pro-overlay-actions">
                                        <div className="pro-btn-circle-alt"><ShoppingBag size={20} /></div>
                                    </div>
    ...
    console.log("Could not find start string in Menu.jsx");
}
...
// Also modify .pro-card to have margin-top so the pull-up works correctly
...
css = css.replace(exactCssOld, exactCssNew);
css = css.replace(proCardOld, proCardNew);

// Adjust pro-price-badge to fit dropping plate properly
